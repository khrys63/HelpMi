<template>
  <div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-2 mb-6 text-sm text-gray-500">
      <router-link :to="`/projects/${projectId}`" class="hover:text-blue-600">← Retour</router-link>
      <span>/</span>
      <span class="text-gray-700 font-medium">Nouveau ticket</span>
    </div>

    <div class="bg-white rounded-xl border border-gray-200 p-6">
      <h1 class="text-xl font-bold text-gray-900 mb-6">Créer un ticket</h1>
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <textarea v-model="form.description" rows="6" placeholder="Étapes pour reproduire, comportement attendu…"
            class="w-full border rounded-lg px-3 py-2 text-sm resize-none focus:ring-2 focus:ring-blue-500 outline-none" />
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Échéance <span class="text-gray-400 font-normal">(facultative)</span></label>
      ...
      <div class="flex justify-end gap-3 mt-6">
        <router-link :to="`/projects/${projectId}`"
          class="px-4 py-2 text-sm text-gray-600 hover:text-gray-900">Annuler</router-link>
